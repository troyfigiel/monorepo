...
** Emacs
+ Bind a bunch of consult commands such as consult-yank, consult-global-mark, etc.
+ Bind consult-find and consult-ripgrep to the project equivalents. They are the drop-in replacements for these functions. Similarly, consult-project-buffer is a replacement for project-switch-to-buffer.
+ Tab-bar-mode can be used to separate different projects. Inside each tab, project can be used to find project-specific files and buffers. I should also look into tab-bar-history-mode.
+ Registers are very useful for window configurations. They can be used to temporarily maximize a buffer to full screen. I am not sure yet what would be a good workflow. This is probably superseded by tab-bar-history-mode.
+ org-indent-mode and avy-goto-line do not work together. There are some solutions in here that I can try out: https://github.com/abo-abo/avy/issues/231.
